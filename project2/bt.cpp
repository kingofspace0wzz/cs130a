...
class BTreeNode
{
    ...
    int t=2;      // Minimum degree (defines the range for number of keys)
...
void BTree::insert(int k)
{
    ...
    if (root == NULL)
    {
        ...
        root->keys[0]->key = k;  // Insert key
        root->keys[0]->counts = 1;
...
void BTreeNode::traverse()
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (leaf == false)
            C[i]->traverse();
        cout << " " << keys[i]->key << ",count: " << keys[i]->key;
    }
...
int main()
{
    BTree t = BTree(); // A B-Tree with minium degree 3
